...
const assertNoUnvalidatedConformanceFixtures = () => {
  ...
  for (const fixturePath of fixturePaths) {
    assert(
      readJsonPaths.has(fixturePath),
      `${fixturePath} is not connected to scripts/validate-artifacts.mts`,
    );
  }
};
...
const assertReservedExtensionNamespaceDrift = () => {
  ...
  const reservedFixture = readJsonFile(
...
validate(
  "advisory",
  readJson("conformance/fixtures/advisory-withdrawn.json"),
  "withdrawn advisory fixture",
);
validate(
  "advisoryList",
  readJson("conformance/fixtures/advisory-list.json"),
  "advisory list fixture",
);
validate(
  "searchResults",
  readJson("conformance/fixtures/search-results.json"),
...
validate(
  "trustSummary",
  readJson("conformance/fixtures/trust-summary.json"),
  "trust summary fixture",
);
validate(
  "trustSummary",
  readJson("conformance/fixtures/trust-summary-empty.json"),
  "empty trust summary fixture",
);
validate("trustDetail", readJson("conformance/fixtures/trust-detail.json"), "trust detail fixture");
validate(
  "trustDetail",
  readJson("conformance/fixtures/trust-detail-empty.json"),
...
const trustDetailStatusVariants = readJson(
...
const capabilityUnknownToleranceFixture = readJson(
...
validate(
  "bridgeMetadata",
  readJson("conformance/fixtures/bridge-metadata.json"),
...
const duplicateComponentFixture = readJson(
...
const siblingPermissionFixture = readJson(
...
const exactReleaseMetadataCases = readJson(
...
const trustArtifactVerificationCases = readJson(
...
const purlVersCompatibilityExceptions = readJson(
...
const componentDependencyCases = readJson(
...
const externalDependencyCases = readJson(
