...
| `POST /api/v1/volumes/{name}/uploads/{uploadId}/finalize`                 | `not-found`, `invalid-manifest`, `invalid-archive`, `digest-mismatch`, `identity-mismatch`, `missing-uploaded-bytes`, `invalid-upload-state`, `idempotency-conflict`, `upload-expired` |
...
| `POST /api/v1/volumes/{name}/{version}/trust/uploads`                     | `authentication-required`, `authorization-failed`, `subject-binding-mismatch`, `payload-too-large`, `unsupported-media-type`                                                           |
| `POST /api/v1/volumes/{name}/{version}/trust/uploads/{uploadId}/finalize` | `missing-uploaded-bytes`, `invalid-upload-state`, `digest-mismatch`, `idempotency-conflict`                                                                                            |
