...
For `blocked`, `tombstoned`, or `unavailable` releases, the bibliotheca returns an RFC 9457 Problem Details response rather than a `dist` block. Do not attempt to install releases in these states via the portable baseline.
