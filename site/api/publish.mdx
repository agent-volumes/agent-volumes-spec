...
All publish endpoints require authentication. See [Authentication](/api/authentication).
...
- Absolute paths, parent-directory traversal (`..`), duplicate normalized paths, and entries that normalize to `.` are invalid
...
8. Checks for permission escalation (components must not declare broader permissions than the parent volume)
...
To yank a published release, use the `DELETE` endpoint. Yanked versions are excluded from ordinary dependency resolution but remain installable when explicitly requested or already pinned. See [Fetch](/api/fetch) for client behavior on yanked releases.
...
The bibliotheca returns `202 Accepted`. The version identity is preserved; its lifecycle state changes to `yanked` or `tombstoned` depending on bibliotheca policy.
