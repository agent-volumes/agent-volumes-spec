...
<ParamField path="volume.name" type="string" required>
  ...
  (`@acme/research-pack`). Name segments must be lowercase alphanumeric with hyphens, 1–128 characters, not starting or
  ending with a hyphen, and without consecutive hyphens. Scoped identifiers also include a scope segment up to 64
  characters, so the full scoped `volume.name` can be longer than a bare name.
</ParamField>
...
<ParamField path="volume.providers" type="string[]">
  ...
  hints, not dependency declarations. See [Compatibility](/volumes/compatibility) for common provider examples and
  naming guidance.
</ParamField>
