...
Declare which agent runtimes your volume supports by adding one `[[runtimes]]` entry per target runtime. If you omit the array entirely, the volume makes no runtime-specific compatibility claim and is treated as potentially compatible with any runtime.
...
<Note>
  Runtime identifiers describe the **execution host**, not the underlying AI model. `claude-code` identifies Anthropic's
  CLI agent; it says nothing about which model that agent is configured to use. Model/provider compatibility is outside
  the v0.1 runtime identifier model.
</Note>

## Protocol compatibility (`[[protocols]]`)

If your volume exports MCP servers or LSP servers, declare protocol compatibility using the `[[protocols]]` array. The `version` field accepts whatever version scheme the protocol ecosystem uses — MCP uses date-like versions, LSP uses short numeric SemVer-compatible versions.
...
<Warning>
  Permission escalation is a validation failure. If a component declares a permission broader than its parent volume,
  clients and bibliothecas reject the artifact. Bibliothecas that discover permission escalation must block the affected
  artifact from continued distribution.
</Warning>
