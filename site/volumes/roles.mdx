...
<Note>
  A `meta` package has no mandatory `[[components]]` entries. Its `[dependencies]` participate in ordinary dependency
  resolution and single-version enforcement like any other volume dependency. The v0.1 core does not assign special
  normative semantics to the full transitive dependency closure of `role = "meta"` packages.
</Note>
