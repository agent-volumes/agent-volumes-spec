...
Each component name must be unique within the volume across all component types. Component names follow the same naming rules as volume names: lowercase letters, digits, and hyphens only.
...
<Tabs>
  <Tab title="Runtimes">
    The `[[runtimes]]` array declares which agent runtimes this volume targets and what version range it's compatible with. If you omit this array entirely, the volume makes no runtime-specific compatibility claim and is considered runtime-neutral.
