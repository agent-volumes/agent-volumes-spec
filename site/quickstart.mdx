...
<Note>
  The `shelf` CLI and the Alexandria bibliotheca are reference implementations currently in development, targeting
  availability in Q2 2026. The manifest format and concepts shown here are stable against the current specification
  draft.
</Note>
...
<Tip>
  For higher assurance, attach SLSA provenance and a Sigstore signature at publish time. See [Supply Chain Security](/security/supply-chain) for the full trust model.
</Tip>
</Step>
