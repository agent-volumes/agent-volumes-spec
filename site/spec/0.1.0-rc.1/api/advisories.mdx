...
<Note>
  Advisory targeting in v0.1 is volume-level only. Component-level impact metadata in `componentImpact` is informational
  and does not narrow or change the normative volume-level advisory target.
</Note>

<Warning>
  Scanner findings are not advisory records by themselves. A bibliotheca may create or update an advisory based on
  scanner information under local policy, but the portable contract is the advisory read and discovery model, not
  scanner-result interchange.
</Warning>
