...
<Warning>
  If your LSP server configuration is at a path other than `.lsp.json`, the runtime or validator will load it
  successfully but emit a `noncanonical-entrypoint` warning. Use `.lsp.json` to ensure predictable discovery.
</Warning>
...
<Note>
  Agent Volumes v0.1 does not define one universal protocol version comparison grammar. Protocol-specific version
  semantics remain advisory in the core spec. Protocol profiles may define stronger comparison semantics in the future.
</Note>
...
<Tip>
  Place your `.lsp.json` at the volume root. The canonical filename convention ensures that tooling can discover your
  LSP server without needing to parse `volume.toml` first, enabling faster runtime integration.
</Tip>
