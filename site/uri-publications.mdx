...
| Bridge and warning companion artifacts     | `bridge-metadata.schema.json`; `warning.schema.json`                        | `schemas/bridge-metadata.schema.json`; `schemas/warning.schema.json`; bridge fixtures; external-dependency warning fixtures                                                    | Versioned schema IDs are immutable after release. Record dereference and validator evidence with the schema publication checks.     |
