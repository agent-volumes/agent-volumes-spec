---
title: 'Inconsistent registry state'
description: 'Stable Problem Details type URI page for inconsistent-registry-state in the Agent Volumes v0.1 Registry API.'
---

`https://agentvolumes.org/problems/inconsistent-registry-state` identifies the `inconsistent-registry-state` Problem Details type in the closed Agent Volumes v0.1 portable problem set.

| Field       | Value                                                           |
| ----------- | --------------------------------------------------------------- |
| Type URI    | `https://agentvolumes.org/problems/inconsistent-registry-state` |
| Slug        | `inconsistent-registry-state`                                   |
| HTTP status | `409`                                                           |
| Title       | `Inconsistent registry state`                                   |
| Meaning     | Index, exact metadata, or trust metadata cannot be reconciled.  |

## Client guidance

Switch on the `type` URI when handling Agent Volumes Registry API errors. The human-readable `title` can vary by implementation or localization, but the `type` URI identifies the portable problem category.

## Canonical sources

- [Problem Details schema](/spec/0.1.0-rc.1/schemas/problem-details.schema.json)
- [Problem registry schema](/spec/0.1.0-rc.1/schemas/problem-registry.schema.json)
- [Problem registry fixture](https://github.com/agent-volumes/agent-volumes-spec/blob/release/0.1.0-rc1/conformance/fixtures/problem-registry.json)
- [API error table](/api/errors)
