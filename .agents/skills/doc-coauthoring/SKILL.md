...
3. **Reader Testing**: Test the doc with a fresh conversation (no context) to catch blind spots before others read it
...
**If no integrations are detected and in the chat interface:** Suggest they can enable connectors in their settings to allow pulling context from messaging apps and document storage directly.
...
**Goal:** Test the document with a fresh conversation (no context bleed) to verify it works for readers.
...
**If access to sub-agents is available (e.g., in a coding environment):**
...
1. Open a fresh conversation in a new window
