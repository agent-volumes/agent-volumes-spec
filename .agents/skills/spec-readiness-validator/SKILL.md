...
description: 'Validate whether a specification document is implementer-ready by checking normative prose, machine-readable schemas, versioning, error semantics, security model, registry behavior, and conformance suite alignment. Use when: (1) Evaluating spec maturity before implementation, (2) Reviewing spec PRs for completeness, (3) Auditing existing specs for gaps, (4) Preparing specs for release, or (5) Any task involving spec quality assessment or readiness evaluation.'
---
...
```bash
python3 scripts/validate_spec.py <path-to-spec-directory>
```
